...
/**
 * Emit a list of all values emitted in the given timeframe (in milliseconds) every
 * [timeframeMillis]
 */
 ...
 *   2, the series will look like this: (2, 1), (4, 2), (6, 3), ...
 */
...
/**
 * Calculate the average message duration of the given flow of Lists of [Event.SysSendmsg] for each
 * list. If the list is empty, the average is 0.
 */
...
/**
 * Calculate the average of bytes written to each file descriptor in the given flow of Lists of
 * [Event.VfsWrite]
 */
...
/**
 * Count the number of file descriptors that are currently open based on the given flow of
 * [Event.SysFdTracking]. This is a count that will be incremented on a SysFdTracking.Created and
 * decremented on a SysFdTracking.Destroyed.
 */
...
/**
 * Convert a flow of [Event.Gc] to a a flow of a pair of used heap size and total heap size for each
 * GC event. This derivation from the GC events is how Perfetto does it.
 */
...
/**
 * Sort the given flow of lists of [Pair] by the second element (y) and clip the result to the given
 * [limit]
 */
