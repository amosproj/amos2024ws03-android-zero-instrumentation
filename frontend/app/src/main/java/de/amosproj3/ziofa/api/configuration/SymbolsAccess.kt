...
interface SymbolsAccess {
     ...
     * Implementation should return a flow that completes once the search is finished.
     *
    ...
    /** Start indexing all symbols of the system into the backend database
     This needs to be done once before searching for symbols.
     Implementations should only index the backend symbols once to avoid indexing the same symbols
     multiple times.
