...
fn get_namespace<'a>(parent: &'a Namespace, name: &'static str) -> Result<&'a Namespace, ParseError> {
    parent.try_get_namespace(name).ok_or(ParseError::MissingNamespace { namespace: name })
}

fn get_type<'a>(parent: &'a Namespace, name: &'static str) -> Result<&'a Type, ParseError> {
    parent.try_get_type(name).ok_or(ParseError::MissingType { ty: name })
}

fn get_field<'a>(parent: &'a Type, name: &'static str) -> Result<&'a Field, ParseError> {
    parent.try_get_field(name).ok_or(ParseError::MissingField { field: name })
}

fn field_to_metadata(field: &Field, name: &'static str, base_offset: usize) -> Result<FieldMetadata, ParseError> {
    let offset = field.try_get_offset().ok_or(ParseError::MissingOffset { field: name })?;
    let ty = field.try_get_type().ok_or(ParseError::MissingType { ty: name })?;
    let size = ty.try_get_size().ok_or(ParseError::MissingSize { field: name })?;
    
    Ok(FieldMetadata { offset: (offset + base_offset) / 8, size })
}
...
pub fn get_includes(base: Option<&str>) -> Vec<String> {
    ...
    INCLUDE_FILES.iter().map(|path| format!("{}{}", prefix, path)).collect()
}
...
pub fn parse(base: Option<String>) -> Result<HeapMetadata, ParseError> {
    ...
    let all_args = get_system_includes().into_iter().chain(get_includes(base.as_deref())).chain(raw_args);
...
pub fn parse_heap_metadata(tu: TranslationUnit) -> Result<HeapMetadata, ParseError> {
    ...
    let current_gc_iteration_offset = current_gc_iteration.try_get_offset().ok_or(ParseError::MissingOffset { field: "current_gc_iteration_" })?;
    let current_gc_iteration_type = current_gc_iteration.try_get_type().ok_or(ParseError::MissingType { ty: "Iteration" })?;
    
    let pause_times = get_field(current_gc_iteration_type, "pause_times_")?;
    let pause_times_offset = pause_times.try_get_offset().ok_or(ParseError::MissingOffset { field: "pause_times_" })? + current_gc_iteration_offset;
    let pause_times_ty = pause_times.try_get_type().ok_or(ParseError::MissingType { ty: "vector<uint64_t>" })?;
    ...
    let freed_offset = freed.try_get_offset().ok_or(ParseError::MissingOffset { field: "freed_" })? + current_gc_iteration_offset;
    let freed_type = freed.try_get_type().ok_or(ParseError::MissingType { ty: "ObjectBytePair" })?;
    ...
    let freed_los_offset = freed_los.try_get_offset().ok_or(ParseError::MissingOffset { field: "freed_los_" })? + current_gc_iteration_offset;
    let freed_los_type = freed_los.try_get_type().ok_or(ParseError::MissingType { ty: "ObjectBytePair" })?;
    ...
    Ok(HeapMetadata {
        ...
        freed_los_objects: field_to_metadata(freed_los_objects, "freed_los_objects", freed_los_offset)?,
