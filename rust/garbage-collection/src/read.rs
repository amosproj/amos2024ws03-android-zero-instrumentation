...
pub trait MemReader {
    ...
    fn read<T>(&self, out: &mut T, offset: usize, len: usize, rel: bool) -> core::result::Result<(), Self::Error>;
}
...
fn read_inner<T, M: MemReader>(field: &mut T, meta: FieldMetadata, reader: &M) -> Result<(), Error<M::Error>> {
    ...
    reader.read( field, meta.offset, size_of::<T>(), true).map_err(Error::ReadError)
}

pub fn read<M: MemReader>(heap: &mut Heap, meta: HeapMetadata, reader: &M) -> Result<(), Error<M::Error>> {
    read_inner(&mut heap.target_footprint, meta.target_footprint, reader)?;
    read_inner(&mut heap.num_bytes_allocated, meta.num_bytes_allocated, reader)?;
    ...
    reader.read::<[u64; 8]>(&mut heap.pause_times, start_ptr, min_len * size_of::<u64>(), false).map_err(Error::ReadError)?;
