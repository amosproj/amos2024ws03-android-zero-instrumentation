...
async fn test_init_stream(client: &mut ZiofaClient<Channel>, verbose: bool) {
    println!("TEST init_stream");
    let stream = client.init_stream(()).await;
    match stream {
        Ok(res) => {
            let res = res.into_inner().message().await;
            match res {
                Ok(Some(ebpf)) => {
                    println!("SUCCESS");
                    if verbose {
                        println!("EbpfStreamObject: {:?}", ebpf)
                    }
                },
                Ok(None) => {
                    println!("ERROR: EBPFStreamObject is None");
                }
                Err(e) => {
                    println!("ERROR: {:?}", e);
                }
...
async fn main() {
    ...
    test_init_stream(&mut client, config).await;
