...
use ractor::{
    cast,
    concurrency::{spawn, JoinHandle},
    factory::{
        queues::DefaultQueue, routing::QueuerRouting, Factory, FactoryArguments, FactoryMessage,
        Job, JobOptions, WorkerBuilder, WorkerMessage, WorkerStartContext,
    },
    Actor, ActorCell, ActorProcessingErr, ActorRef, RpcReplyPort, SpawnErr, State,
    SupervisionEvent,
};
use tantivy::{
...
use super::{
    index::index,
    symbolizer::{symbols, Symbol},
    walking::{all_symbol_files, SymbolFilePath},
};
...
where
    S: Stream<Item = SymbolFilePath> + State + Unpin,
{
    type Msg = ();
...
impl Actor for SymbolFileParserProxy {
    ...
    async fn pre_start(
        ...
        let factory_def = Factory::<
            (),
            SymbolFilePath,
            ActorRef<SymbolWithPath>,
            SymbolFileParser,
            QueuerRouting<(), SymbolFilePath>,
            DefaultQueue<(), SymbolFilePath>,
        >::default();
        ...
        let (actor_ref, _) = Actor::spawn_linked(
            Some("symbol-file-parser-factory".to_owned()),
            factory_def,
            factory_args,
            args.2.get_cell(),
        )
...
impl WorkerBuilder<SymbolFileParser, ActorRef<SymbolWithPath>> for SymbolFileParserBuilder {
    fn build(
        &mut self,
        _: ractor::factory::WorkerId,
    ) -> (SymbolFileParser, ActorRef<SymbolWithPath>) {
...
impl Actor for SymbolFileParser {
    ...
    async fn handle(
        ...
        match message {
            WorkerMessage::FactoryPing(instant) => Ok(cast!(
                state.factory,
                FactoryMessage::WorkerPong(state.wid, instant.elapsed())
            )?),
...
pub type SearchRes = Result<Vec<shared::ziofa::search_symbols_response::Symbol>, io::Error>;
    ...
    async fn handle(
        ...
        match message {
            ...
            SymbolActorMsg::Search(SearchReq { query, limit }, reply) => {
                ...
                let query =
                    match QueryParser::for_index(state, vec![symbol_name]).parse_query(&query) {
                        Ok(query) => query,
                        Err(e) => {
                ...
                let results = searcher
                    ...
                    .map(|(_, address)| {
                        let doc: TantivyDocument =
                            searcher.doc(address).map_err(io::Error::other)?;
                        let name = doc
                            .get_first(symbol_name)
                            .and_then(|x| x.as_str())
                            .ok_or_else(|| io::Error::other("expected str"))?;
                        let path = doc
                            .get_first(library_path)
                            .and_then(|x| x.as_str())
                            .ok_or_else(|| io::Error::other("expected str"))?;
                        let offset = doc
                            .get_first(symbol_offset)
                            .and_then(|x| x.as_u64())
                            .ok_or_else(|| io::Error::other("expected u64"))?;

                        Ok::<_, io::Error>(shared::ziofa::search_symbols_response::Symbol {
            ...
            SymbolActorMsg::GetOffset(
                ...
                let symbol_query = TermQuery::new(
                    Term::from_field_text(symbol_name_exact, &symbol_name),
                    tantivy::schema::IndexRecordOption::Basic,
                );
                let library_path_query = TermQuery::new(
                    Term::from_field_text(library_path_exact, &library_path),
                    tantivy::schema::IndexRecordOption::Basic,
                );
                let query = BooleanQuery::intersection(vec![
                    Box::new(symbol_query),
                    Box::new(library_path_query),
                ]);
...
async fn spawn_symbol_file_path_collector(
    destination: ActorRef<SymbolFilePath>,
    ...
    let (actor_ref, _) = Actor::spawn_linked(
        Some("symbol-file-path-collector".to_owned()),
        SymbolFilePathCollector(PhantomData),
        (symbol_files, destination),
        sup,
    )
    .await?;
...
async fn spawn_symbol_file_parser(
    destination: ActorRef<SymbolWithPath>,
    num: usize,
    supervisor: Option<ActorCell>,
) -> Result<ActorRef<SymbolFilePath>, SpawnErr> {
    let sup = supervisor.unwrap_or_else(|| destination.get_cell());

    let (actor_ref, _) = Actor::spawn(
        Some("symbol-file-parser-supervisor".to_owned()),
        SymbolFileParserProxy,
        (num, sup, destination),
    )
    .await?;
...
async fn spawn_symbol_indexer(
    writer: Arc<IndexWriter>,
) -> Result<(ActorRef<SymbolWithPath>, JoinHandle<()>), SpawnErr> {
    let (actor_ref, handle) =
        Actor::spawn(Some("symbol-inderer".to_owned()), SymbolIndexer, writer).await?;
