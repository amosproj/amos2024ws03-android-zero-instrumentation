...
fn u64_from_hex_str<'de, D>(de: D) -> Result<u64, D::Error>
where
    D: Deserializer<'de>,
{
...
pub async fn oat_symbols<P: AsRef<Path>>(
    path: P,
    tx: flume::Sender<Symbol>,
) -> Result<(), io::Error> {
    ...
    let mut stream = ProcessLineStream::try_from(cmd)?
        ...
        .map(move |symbol| Symbol {
            name: symbol.name,
            offset: symbol.offset + oatdata_offset,
        });
...
pub async fn so_symbols<P: AsRef<Path>>(
    path: P,
    tx: flume::Sender<Symbol>,
) -> Result<(), io::Error> {
    let file = AsyncMmapFile::open(path).await.map_err(io::Error::other)?;
    ...
    for symbol in obj.dynamic_symbols() {
        let name = if let Ok(name) = symbol.name() {
            name
        } else {
            continue;
        };
...
pub async fn symbols(path: SymbolFilePath, tx: flume::Sender<Symbol>) -> Result<(), io::Error> {
    match path {
        SymbolFilePath::Odex(path_buf) | SymbolFilePath::Oat(path_buf) => {
            oat_symbols(path_buf, tx).await
        }
